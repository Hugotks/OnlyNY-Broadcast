...
(function (bodyScrollLock, themeAddresses, themeCurrency, Sqrl, themeImages, Flickity, FlickityFade, Rellax, ellipsed, AOS) {
  ...
  class ProductForm {
    constructor(element, product, options) {
      ...
      this.color = element.parentNode.parentNode.parentNode.dataset.color;
    ...
    variant(flag) {
      ...
      if (flag) {
        opts.push({name:'Color', value:this.color});
      }
      if (opts.length) {
    ...
    getFormState(flag = false) {
      const variant = this.variant(flag);
      return {
    ...
    _onFormEvent(cb, flag) {
      if (typeof cb === 'undefined') {
        return Function.prototype.bind();
      }

      return function (event) {
        event.dataset = this.getFormState(flag);
        this._setIdInputValue(event.dataset.variant);
        cb(event);
      }.bind(this);
    }
    ...
    _initInputs(selector, cb) {
      ...
      return elements.map(
        function (element) {
          if (element.classList.value.indexOf("is_quick_add") > -1) {
            flag = true;
          }
          this._listeners.add(element, 'change', this._onFormEvent(cb, flag));
          return element;
        }.bind(this)
      );
    }
    ...
    getForm() {
      ...
      fetch(root + '/products/' + this.quickAddLabel.getAttribute(selectors$g.elements.quickAddLabel) + '?section_id=api-quick-add')
        ...
        .then((data) => {
          ...
          const unique = `${collectionHandle}-${this.sectionId}-index${rand}`;
          this.quickAddFormHolder.innerHTML = cleanResponse.replaceAll('||collection-index||', unique);

          const info = this.quickAddFormHolder.parentElement.parentElement.previousElementSibling.querySelectorAll('option');
          this.quickAddFormHolder.querySelectorAll('.radio__button input').forEach((el) => {
            info.forEach((e) => {
              if (el.value == e.dataset.option2 && e.value == '1') {
                el.nextElementSibling.classList.add("out_p");
              }
            });
          });
    ...
    submitForm(e, replaceHref = '') {
      ...
      if (this.productsContainer) {
        ...
        fetch(`${window.location.pathname}${window.location.search}`)
          .then((response) => response.text())
          .then((data) => {
            var innerResult = new DOMParser().parseFromString(data, 'text/html').querySelector(selectors$i.productsContainer).innerHTML;

            if (localStorage.getItem('productGrid') === '2') {
              innerResult = innerResult.replaceAll('large-up--one-quarter', 'large-up--one-half');
            }
            this.productsContainer.innerHTML = innerResult;
            ...
            products.forEach((item) => {
              if (colors.length && !colors.includes(item.dataset.color)) {
  ...
  class Collection {
    ...
    init() {
      ...
      this.document.addEventListener('click', function (event) {
        ...
        if (this.collectionSidebar && !this.collectionSidebar.contains(clickedElement) && this.collectionSidebar.classList.contains("expanded") && !this.collectionSidebar.classList.contains("expanding")) {
          this.hideSidebar();
        }
      }.bind(this));
  ...
  class Drawer {
    ...
    closers() {
      ...
      this.underlay.addEventListener('click', () => {
        document.querySelectorAll('[data-sliderule]').forEach((el) => {
    ...
    sliderThumbs(thisEl) {
      ...
      if (this.thumbs && slides.length) {
        ...
        slides.forEach((element, i) => {
          ...
          const color = slide.getAttribute(selectors$w.dataColor);
          let thumbAlt = '';
          const thumbIcon = thumbIcons[type] ? thumbIcons[type] : '';
          let selected = '', style = thisEl.element.dataset.flag == '1' ? '' : 'display: none;';
  ...
  const selectors$E = {
    ...
    productSlideshow: '[data-product-slideshow]',
    productThumbItem: '[data-thumb-item]',
      ...
      if (variant) {
        ...
        if (variant.featured_media) {
          ...
          if (newImageParent) {
            ...
            this.container.querySelectorAll(selectors$E.productThumbItem).forEach((el) => {
              if (el.dataset.color === variant.option1) {
